...
- 이때 자신이 구현한 코드가 컴퓨터 내부에서 어떻게 동작할 것인지 예측 가능해야 하며, 이를 명확히 설명할 수 있어야 합니다.
  - **기본 개념과 동작 원리를 이해하지 못한 상태에서 복사 붙혀넣기로 단순히 동작만 하는 코드를 만들고 거기에 만족한다면 여러분이 구현한 코드는 언제 무너져도 이상할 것이 없는 사상누각일 뿐입니다.**
...
- 간단히 말하자면 변수는 프로그래밍 언어에서 값을 저장하고 잠조하는 메커니즘으로, 값의 위치를 가리키는 상징적인 이름입니다.
...
- 자바스크립트 엔진에서 변수 선언은 2단계에 거쳐 수행됩니다.
...
- 문은 여러 토큰으로 구성됩니다.
  - 토큰이랑 문법적인 의미를 가지며, 문법적으로 다 이상 나눌 수 없는 코드의 기본 요소를 의미합니다.
...
> 용어정리 🧐
>
> - 프로퍼티: 프로퍼티란 키와 값 (key value)를 연경하는 객체의 구성원 입니다.
...
- 일치 비교 연산자에서 주의해야 할 것이 몇가지 있습니다.
  ...
  - NaN은 자신과 일차하지 않는 유일한 값입니다.
  - 따라서 숫자가 NaN인지 조사하려면 빌트인 함수 `Number.isNaN`을 사용해야 합니다

- 숫자 0 또한 주의해아 햡니다.
  - 자바스크립테에는 양의 0과 음의 0이 있는데 이 둘을 비교하면 true를 반환합니다.
  ```js
  0 === -0; // true
  ```
...
- 아래 내용은 React Deep Dive 내용 일부 발췌 입니다. Obuject.is 메서드는 리액트를 학습하기 이전에 알아놓으면 좋을거같아서 가져왔어요

## 자바스크립트의 동등 비교

- 리액트 컴포넌트의 렌더링이 일어하는 이유 중 하나가 바로 Props의 동등 비교에 따른 결과 입니다.
  - props의 동등 비교는 객체의 같은 비교를 기반으로 이뤄지는데, 이 얕은 비교가 리액트에서 어떻게 작동하는지 이해하지 못하면...
